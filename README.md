...
| `funding` | Each statement, then each award as "name (id)", in the margin rail under the correspondence. Write `funding` as a list, because mystmd, through 1.11.0, stops with `funding?.forEach is not a function` on a single funding object | Omitted |
...
| Download | Each entry of `downloads` with a web address, by its `title`. Give the artifact its own name there, as in "Latest PDF", "Slides", "BibTeX". The column already says Download, so a title repeating that word is redundant. A qualifier such as "Latest" is a claim about the address beside it, so it holds exactly as long as that address tracks a branch, and goes stale the moment an entry is pinned to a tag or a version. Archiving the paper does not break it: a branch-tracking link inside a frozen record still resolves to what the branch holds, which is the same promise the concept DOI in "Cite as" makes. MyST announces a pdf download as "Download as pdf" ahead of the title, so a title ending in PDF repeats the word for a screen reader. An entry whose address ends in `.bib` goes to "Cite as" as a BibTeX link instead. See The site for the second list a site needs |
