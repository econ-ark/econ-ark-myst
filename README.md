...
| Download | Each entry of `downloads` with a web address, by its `title`. Give the artifact its own name there, as in "Latest PDF", "Slides", "BibTeX". The column already says Download, so a title repeating that word is redundant. A qualifier such as "Latest" is a claim about the address beside it, so it holds exactly as long as that address tracks a branch, and goes stale the moment an entry is pinned to a tag or a version. Archiving the paper does not break it: a branch-tracking link inside a frozen record still resolves to what the branch holds, which is the same promise the concept DOI in "Cite as" makes. On a site MyST reads the format itself and announces the link as "Download as pdf", so a title ending in PDF says it twice to a screen reader; that is the price of the word being useful in the PDF's own column. An entry whose address ends in `.bib` goes to "Cite as" as a BibTeX link instead. See The site for the second list a site needs | Each entry of `downloads` with a web address, by its `title`. An entry whose address ends in `.bib` goes to "Cite as" as a BibTeX link instead. See The site for the second list a site needs |
